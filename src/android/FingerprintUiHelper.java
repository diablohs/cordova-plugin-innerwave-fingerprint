...
public class FingerprintUiHelper extends FingerprintManager.AuthenticationCallback {
    ...
    public void startListening(FingerprintManager.CryptoObject cryptoObject) {
        ...
        int ic_fp_40px_id = mContext.getResources()
                .getIdentifier("ic_fp_40px", "drawable", FingerprintPlugin.packageName);
    ...
    public void onAuthenticationFailed() {
        ...
        if (mAttempts > FingerprintPlugin.mMaxAttempts) {
    ...
    public void onAuthenticationSucceeded(FingerprintManager.AuthenticationResult result) {
        ...
        int ic_fingerprint_success_id = mContext.getResources()
                .getIdentifier("ic_fingerprint_success", "drawable", FingerprintPlugin.packageName);
        mIcon.setImageResource(ic_fingerprint_success_id);
        int success_color_id = mContext.getResources()
                .getIdentifier("success_color", "color", FingerprintPlugin.packageName);
        mErrorTextView.setTextColor(
                mErrorTextView.getResources().getColor(success_color_id, null));
        int fingerprint_success_id = mContext.getResources()
                .getIdentifier("fingerprint_success", "string", FingerprintPlugin.packageName);
    ...
    private void showError(CharSequence error) {
        int ic_fingerprint_error_id = mContext.getResources()
                .getIdentifier("ic_fingerprint_error", "drawable", FingerprintPlugin.packageName);
        mIcon.setImageResource(ic_fingerprint_error_id);
        mErrorTextView.setText(error);
        int warning_color_id = mContext.getResources()
                .getIdentifier("warning_color", "color", FingerprintPlugin.packageName);
        mErrorTextView.setTextColor(
    ...
    Runnable mResetErrorTextRunnable = new Runnable() {
        ...
        public void run() {
            int hint_color_id = mContext.getResources()
                    .getIdentifier("hint_color", "color", FingerprintPlugin.packageName);
            mErrorTextView.setTextColor(
                    mErrorTextView.getResources().getColor(hint_color_id, null));
            int fingerprint_hint_id = mContext.getResources()
                    .getIdentifier("fingerprint_hint", "string", FingerprintPlugin.packageName);
            mErrorTextView.setText(
